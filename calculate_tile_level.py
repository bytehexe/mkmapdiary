...
def object_level(points):
    ...
    if len(pts) == 2:
        ...
        central_angle = np.arccos(
            np.clip(sin_lat[:, None]*sin_lat[None, :] +
                    cos_lat[:, None]*cos_lat[None, :]*np.cos(dlon), -1, 1)
        )
    ...
    level = int(round(math.log(360. / degrees * adjustment_factor)))
